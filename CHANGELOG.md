...
- Added sender-consent voice transcription (migrations `027_message_transcription_consent` + `028_message_transcript_commands`): sends carry `transcriptionConsent`, `PUT /v1/messages/:id/transcript` attaches one receiver-made transcript to consenting voice/audio messages with first-nonce-wins replay semantics, message projections expose `transcriptionAllowed`/`transcript`, and a `message.updated` event converges members. Transcripts are server-readable by design. Evidence: protocol 13 files/95 tests PASS; API 74 files/637 tests PASS.
- Added text-only scheduled send (migration `029_scheduled_messages`): bounded horizons, per-chat pending list, owner cancel, and a 30-second dispatcher replaying live-send guards with bounded failure codes. Evidence: protocol 14 files/97 tests PASS; API 75 files/639 tests PASS.
